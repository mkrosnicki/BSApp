...
class ApiProvider {
  ...
  Future<dynamic> _sendRequest(_RequestType requestType, String endpoint,
      {Map<String, dynamic> body, String token, Map<String, dynamic> requestParams}) async {
    var responseJson;
    try {
      ...
      switch (requestType) {
        ...
        case _RequestType.GET:
          ...
          break;
        default:
          throw IllegalRequestException('Request $requestType is not supported!');
      }
  ...
  Uri _buildUri(String endpoint, Map<String, dynamic> requestParams) {
    return _protocolType == _ProtocolType.HTTP
        ? Uri.http(domain, endpoint, requestParams)
        : Uri.https(domain, endpoint, requestParams);
  }
  ...
  dynamic _response(http.Response response) {
    switch (response.statusCode) {
      ...
      default:
        throw FetchDataException('Error occurred while Communication with Server with StatusCode : ${response.body}');
    }
  }
}
